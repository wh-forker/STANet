close all;
clear all;
fidp = 'D:\wgt\AVE_master_test\fenlei.txt';
[path, prd, keneng, label] = textread(fidp, '%s%s%s%s', 'delimiter', '+');
ImgIndex = 140001;
LengthFiles = length(path);
crop_path = 'F:\wgt\Crop_data\crop\';
rgb_path = 'F:\wgt\Crop_data\rgb\';
txt_path = 'F:\wgt\Crop_data\txt\';

rect = [];
while(ImgIndex<=160000)
      CImgIndex = ImgIndex;
      if strcmp(label(CImgIndex), prd(CImgIndex)) & str2double(keneng(CImgIndex))>0.99
          if(CImgIndex<=LengthFiles)
               imgpath = char(path(CImgIndex, :));
                path_len = length(dir(['D:\wgt\AVE_master_test\result5' imgpath(1:end-10)]));
                ...
                V_path = ['D:\wgt\AVE_master_test\result5' imgpath(1:end-10) num2str(index, '%04d') '_c.jpg'];
                index
